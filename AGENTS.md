...
**Alta y baja de vínculos multi-pieza (F3-3) siguen el mismo criterio local-only que CARA y DIENTE — todavía no llaman a `setVinculo`/`removeVinculo` (eso es F4-1).** Confirmar el picker en modo `MULTI` mete el vínculo en el mismo `useState` de `vinculos` que ya lee `OdontogramaGrid`, con una clave `local-${Date.now()}` (el id real, cuando se conecte F4-1, lo genera `push()` del lado del service — no hay que anticipar ese formato acá). Borrar un vínculo **no pasa por el picker**: `VinculoSpan.tsx` es un `<button>`, no un `<div>`, y clickear el propio grafismo del span llama directo a `onQuitarVinculo(id)` — sin diálogo de confirmación, mismo criterio que "Quitar hallazgo" en `HallazgoPicker`. `tieneHallazgos()` sigue sin mirar `vinculos` a propósito: una pieza dentro de un tramo sin hallazgos propios se dibuja limpia, el tramo lo pinta el vínculo.
