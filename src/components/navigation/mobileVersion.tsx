...
export function MobileVersion({ openLogOutAlert, setOpenLogOutAlert }: props) {
  ...
  return (
    <div>
        ...
        <div className="flex px-1.5 py-3 items-center justify-between">
          <div>
            <BsArrowBarRight
              ...
              size={40}
              className=" bg-white rounded-full bg-opacity-10 hover:bg-opacity-15 hover:border-opacity-70 border-2 border-transparent hover:border-white transition duration-150 cursor-pointer p-1"
            />
          </div>
          {data ? (
            <div className="flex items-center justify-end w-full">
              ...
              <div
                ...
                className={`${openUserMenu ? "rounded-t-xl border-white border-opacity-70 " : "hover:border-white hover:border-opacity-70  rounded-full hover:bg-opacity-15"} flex px-2 cursor-pointer w-44 justify-center relative  items-center mr-2  border-transparent border-2 bg-white bg-opacity-10  transition duration-150 py-1  `}
              >
                <p className="w-full flex justify-start text-sm font-medium select-none text-nowrap">
                  {data.displayName}
                </p>
                {openUserMenu ? (
                  <>
            ...
            <div className="flex items-center gap-2 animate-pulse">
              ...
              <div className="w-40 h-8 rounded-xl bg-white bg-opacity-10 mr-2" />
      ...
      <div
        ref={sidebarRef}
        className={`fixed  top-0 left-0 z-50 h-full  transition-all duration-150  border-r-4 border-teal-700 ${openSideBar ? " translate-x-fit w-[60%]" : "-translate-x-full"}   bg-teal-900	`}
        aria-label="Sidebar"
      >
        <div className="h-full   font-medium overflow-y-auto tracking-tight">
          <div className="w-full border-b-4 pl-1 pr-1.5 py-3  bg-teal-950  border-teal-700 mb-4 flex justify-between">
            <div className="flex items-center  ml-1 select-none mt-1 mb-1 font-bold ">
              ...
              <span className="bg-teal-600 px-1.5  rounded-xl ml-0.5 text-sm">
                PANEL
              </span>
            </div>
            <BsArrowBarLeft
