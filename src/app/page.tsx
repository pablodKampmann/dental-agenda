...
export default function Page() {
  ...
  useEffect(() => {
    ...
    setDayName(dn);
    setDayNum(today.toLocaleDateString('es-AR', { ...options, day: 'numeric' }));
  ...
  useEffect(() => {
    ...
    let dn = dateToday.toLocaleDateString('es-AR', { ...options, weekday: 'long' });
    dn = dn.charAt(0).toUpperCase() + dn.slice(1);
    const dNum = dateToday.toLocaleDateString('es-AR', { ...options, day: 'numeric' });
    let mn = dateToday.toLocaleDateString('es-AR', { ...options, month: 'long' });
    mn = mn.charAt(0).toUpperCase() + mn.slice(1);
  ...
  useEffect(() => {
    if (appointmentDate) {
      ...
      if (!appointments || appointments.length === 0) {
        const maxSlots = slots.filter(s => (TIME_SLOTS as readonly string[]).includes(s)).length;
  ...
  function handleCliclRow(time: string, event: any) {
    if (!isLoadAppoints) {
      if (appointments && appointments.some((a: { time: string }) => a.time === time)) {
        clean();
        const appointment = appointments.find((a: { time: string }) => a && a.time === time);
        setAppointmentSelect(appointment);
        setOpenModalAppointment(true);
        const modalHeight = 140;
        const spaceBelow = window.innerHeight - event.clientY;
        setMousePosition({ x: event.pageX, y: event.pageY, flipUp: spaceBelow < modalHeight });
  ...
  async function handleSetAppoint(patientId: number, dateData: any, reason: any, observations?: string) {
    setIsLoadAppoints(true);
    clean();
                  ...
                  <div className='select-none absolute bg-white text-black border-2 border-gray-600 rounded-lg top-10 z-10'>
                    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="es">
                      <DemoContainer components={['DateCalendar']}>
                        <DateCalendar value={calendarValue} onChange={(newValue) => setCalendarValue(newValue)} views={['day', 'year']} />
                      </DemoContainer>
